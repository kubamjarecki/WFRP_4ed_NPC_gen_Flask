...
class Abilities:
    """Klasa która wylicza, które umiejętności powinna mieć postać,
    musi zostać użyta po określeniu rasy, profesji i levelu postaci.
    Ostatecznie dodaje do instancji Character słownik z umiejetnościami
    w kluczach i ich wartościami liczbowymi w wartościach.
    """

    def __init__(self):
        self.umiejki_str = None
        self.umiejki = None
    ...
    def ask_for_race_abilities(self):
        ...
        if wybor_umiejek == "t":
            wh = 1
            while wh == 1:
                ilosc_umiejek = (len(self.umiejki) - 1)
                zasieg_um = []
                for key, value in self.umiejki_dict.items():
                    zasieg_um.append(str(key))

                in_um5a = input(
                    f"{self.umiejki_str} \n\nWybierz pierwszą umiejętność, która będzie na poziomie 5 (0-{ilosc_umiejek})")
                in_um5b = input(
                    f"Wybierz drugą umiejętność, która będzie na poziomie 5 (0-{ilosc_umiejek}) ")
                in_um5c = input(
                    f"Wybierz trzecią umiejętność, która będzie na poziomie 5 (0-{ilosc_umiejek}) ")
                in_um3a = input(
                    f"Wybierz pierwszą umiejętność, która będzie na poziomie 3 (0-{ilosc_umiejek}) ")
                in_um3b = input(
                    f"Wybierz drugą umiejętność, która będzie na poziomie 3 (0-{ilosc_umiejek}) ")
                in_um3c = input(
                    f"Wybierz trzecią umiejętność, która będzie na poziomie 3 (0-{ilosc_umiejek}) ")
                umiejkix = {}
                ...
                if len(umiejkix) != 6:
                    print(
                        f"Coś zjebałeś. Spróbuj jeszcze raz. Pamiętaj że musisz wybrać liczbę z przedziału 0 - {ilosc_umiejek}, oraz że nie możesz dwa razy wybrać tej samej umiejętności")
